...
- Homepage hero: `2400 × 1600` minimum, landscape, under 600 KB after optimization when possible
...
Keep text, logos, and important faces away from outer crop zones. Record source, owner, license, approval date, required credit, and restrictions before setting a non-null `src` in `src/lib/media.ts`.

## Activation rule

Do not set a media `src` in `src/lib/media.ts` until the file exists and the rights record is complete. If a listed file is missing, VibeVI should continue showing polished CSS/SVG fallback art rather than a broken image or public placeholder text.

See `docs/media-acquisition-checklist.md` and `docs/media-drop-in-map.md` for the launch shot list, rights evidence fields, and pre-launch media QA checks.
