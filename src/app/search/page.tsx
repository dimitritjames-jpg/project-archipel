...
export default async function SearchPage({ searchParams }: Props) {
  const { vibe, island, category, q = "" } = await searchParams;
  const query = q.trim();
  ...
  const searchResults = hasQuery
    ? searchPublicInfoCatalogWithScope(query, {
        islandSlug: scopedIsland ? island : null,
    ...
    <>
        ...
        <section className="section-shell grid gap-10 px-4 py-20 sm:px-6 lg:grid-cols-[1fr_0.48fr] lg:items-end lg:py-28">
          <div>
            ...
            <div className="mt-8 max-w-2xl">
              <IslandAskBar
                key={`${query}-${island ?? "all"}-${category ?? "all"}`}
                variant="compact"
                defaultQuery={query}
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {query ? (
                  <p className="text-sm text-archipel-white/62">
                    {scopeSummary
                      ? `Live results for "${query}" in ${scopeSummary}`
      ...
      <div className="section-shell py-12 sm:py-16 lg:py-20">
              ...
              <div>
                ...
                {scopeSummary ? (
                  <div className="mt-4 flex flex-wrap gap-2">
                    {scopedIsland ? (
                      <span className="rounded-full border border-[#0b4b55]/10 bg-white px-3 py-1 text-[11px] font-semibold text-[#45636a]">
                        Island: {scopedIsland.name}
                      </span>
                    ) : null}
                    {scopedCategory ? (
                      <span className="rounded-full border border-[#0b4b55]/10 bg-white px-3 py-1 text-[11px] font-semibold text-[#45636a]">
                        Category: {scopedCategory.name}
                      </span>
                    ) : null}
                  </div>
                ) : null}
                ...
                >
                  {isOwnerIntent
                    ...
                    : scopeSummary
                      ? `Results for "${query}" in ${scopeSummary}`
                      : `Results for "${query}"`}
                </h2>
              ...
              <div className="flex flex-wrap gap-2">
                {scopeSummary ? (
                  <Link
                    ...
                    className="rounded-full border border-aqua/20 bg-aqua/8 px-4 py-2 text-sm font-semibold text-[#0b4b55] transition hover:-translate-y-0.5 hover:border-aqua/35"
                  >
                    Search all islands
                  </Link>
                ) : null}
        ...
        <section
          ...
          aria-labelledby="category-chips"
        >
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {CORE_CATEGORIES.map((categoryOption) => (
              <Link
                key={categoryOption.slug}
                href={`/st-thomas/${categoryOption.slug}`}
        ...
        <section
          className="mt-16 border-t border-white/8 pt-12"
          aria-labelledby="utility-search"
        >
          <SectionHeader
