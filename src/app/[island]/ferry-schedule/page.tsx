...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonical = `${env.NEXT_PUBLIC_SITE_URL}/${islandParam}/ferry-schedule`;

  return {
    title: `Ferry Schedule — ${name}`,
    description: `Verified ferry departures and Next Boat countdown for ${name}.`,
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
      ...
      <p className="mt-4 text-archipel-white/70">
        Next Boat widget and route data connect in Phase 2.
      </p>
    </div>
  );
}
